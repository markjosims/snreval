function S = nist_stnr_m(D,SR,doplot)
  ...
  % calculate power, assueming short samples
  D2 = (D*16384).^2;
  
  nhops = floor(length(D2)/frame_adv);
  % ADDED BY Mark J Simmons Sep 5 2024
  % Sometimes nhops*frame_adv fell outside the length of the array
  % If so, decrement nhops by 1
  if nhops*frame_adv>length(D2)
    nhops=nhops-1;
  end
  ...
  while (10*smoothed_hist(1+first_peak)) < max_val
    first_peak = locate_extremum(smoothed_hist,starting_point,BINS,PEAK);
    last_start = starting_point;
    starting_point = first_peak+1;
    % ADDED BY Mark J Simmons on 5 Sep 2024
    % if first peak predicted at same point twice, avoid infinity loop and return 0
    if last_start==starting_point
      S = 0;
      return;
    end
  end
